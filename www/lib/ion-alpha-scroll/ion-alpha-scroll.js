...
.directive('ionAlphaScroll', [
    '$ionicScrollDelegate', '$location', '$timeout', '$document','$rootScope',
    function($ionicScrollDelegate, $location, $timeout, $document,$rootScope) {
        return {
            require: '?ngModel',
            restrict: 'AE',
            replace: true,//toArray:false | filter:filterlist(sorted_items,data.value) |custom:data.value
            compile: function(tElement, tAttrs, tTransclude) {
                ...
                var template = angular.element([
                    ...
                    '<ion-item ng-repeat="item in items" class="item-remove-animate item-icon-right" type="item-text-wrap" ng-click="showDetails(item.objIndexID)"></ion-item>',
                    '</div>',
                    '<ion-item class="item-remove-animate item-avatar item-icon-right" type="item-text-wrap" style="border: 0px;"><div></div></ion-item>',
                    '</ion-scroll>',
